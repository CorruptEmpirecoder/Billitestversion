...
async def alip(client: Client, message: Message):
    ...
    man = (
        ...
        f"{emoji} <b>𝙱𝙸𝙻𝙻𝙸 𝙱𝙾𝚃 𝚅𝙴𝚁𝚂𝙸𝙾𝙽:</b> <code>{BOT_VER}</code> \n"
        f"{emoji} <b>𝙱𝙸𝙻𝙻𝙸 𝙼𝙾𝙳𝚄𝙻𝙴𝚂:</b> <code>{len(modules)} Modules</code> \n"
        f"{emoji} <b>𝙱𝙸𝙻𝙻𝙸 𝙿𝚈 𝚅𝙴𝚁𝚂𝙸𝙾𝙽:</b> <code>{python_version()}</code> \n"
        f"{emoji}✧[𝙶𝚁𝙾𝚄𝙿](https://t.me/THEROOMOFSECRET)** \n" 
        f"{emoji}✧[𝙲𝙷𝙰𝙽𝙽𝙴𝙻](https://t.me/BLOODAVI)** \n"
